...
Champs : Status (Todo / In Progress / Done — **le code utilise les options par index** : 0 = nouveau ticket, 1 = évalué ; renommer les colonnes dans l'UI GitHub ne casse rien), Value (High/Medium/Low), Effort (XS/S/M/L/XL), Priority (number), Account (text), Revenue at stake (text). Ces 5 champs ont été créés par nous via l'API ; les afficher sur la face des cards : vue Board → ⋯ → Fields.
...
Base `appeGLmP4EhvvujVY` (ex-"Claro", renommée par Valentin ; contient aussi une table `Agences` d'un autre projet — ne pas y toucher). Tables créées par nous via le connecteur :
- `Accounts` : Name, Plan (Enterprise/Growth), ARR (€), Seats, Renewal date, Health (Healthy/At risk/Churning), Requests logged, Usage notes, Cohort size — 2 lignes (Meridian Brokers, Lyon Textile Co.)
...
L'app lit les deux tables **par nom** via l'API REST, avec un token `data.records:read` limité à cette base, saisi dans le panneau opérateur. Airtable est prioritaire sur `context/` ; le contexte est rechargé avant chaque évaluation. Geste de démo : changer Health ou ARR de Meridian dans Airtable → "Evaluate last ticket again" → la card change sur GitHub.
...
- `ACCOUNTS` : `broker` = Meridian Brokers (gros compte bruyant, renouvellement dans 6 semaines), `shipper` = Lyon Textile Co. (1 des 43 comptes silencieux). `AC` = compte courant
...
- `SYSTEM_PROMPT`, `validate()`, `priority()`, `callModel()`, `evaluate(ticket)` : gpt-4o-mini, `response_format: json_object`, **un retry** avec les erreurs du validateur, puis abandon loggé
...
- Exporté : `AG.submit, again, pickImportance, connect, evaluateLast, toggleOps, switchAccount, events, summary`
...
Priorité calculée en JS, pas par le modèle : `raw = reach·impact·confidence·business_fit / effort ; priority = round(100·raw/(raw+300))`.

### Vue opérateur (cachée du client)
`Cmd/Ctrl + .` dans l'app, ou ouvrir le fichier avec `#ops` dans l'URL. Contient : clé OpenAI, owner GitHub (`valentinpotie`), n° de projet (`4`), token GitHub (`gh auth token` a déjà le scope `project`), bouton Connect, état de l'agent, tuiles de télémétrie, événements bruts, Activity.
